...
impl NewMasterTaskRequest {
    pub fn validate(&self) -> Vec<ValidationError> {
        ...
        if self.master_plan_id.trim().is_empty() {
            errors.push(ValidationError::new("master_task_id", "Master Task Id is a must."));
        }

        if self.abstract_task_id.trim().is_empty() {
            errors.push(ValidationError::new("abstract_task_id", "Abstract Task Id is a must"));
        }

        if self.duration <= 0 {
            errors.push(ValidationError::new("duration", "should be a minimum of 1 minute."));
        }
        ...
        if self.task_type.trim().is_empty() {
            errors.push(ValidationError::new("task_type", "Please provide Task Type"));
        }
